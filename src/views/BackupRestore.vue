  ...
  <div style="max-width: 500px">
    <el-scrollbar ref="scrollbar" max-height="400px" :native="true">
      <el-row v-for="file in listFile" :key="file" justify="center">
        <el-button-group>
          <el-button class="select" type="info" size="medium" @click="choose(file)">{{ file }}</el-button>
          <el-popconfirm title="Are you sure to delete this?" @confirm="deleteBackup(file)">
            <template #reference>
              <el-button size="medium" type="danger" :icon="Delete"></el-button>
            </template>
          </el-popconfirm>
        </el-button-group>
      </el-row>
    ...
    <el-row :gutter="10" justify="center">
      <el-button size="medium" type="primary" @click="createBackup">Create</el-button>

      <el-button size="medium" type="success" @click="download" :disabled="!choice">
        Download<el-icon class="el-icon--right"><Download /></el-icon>
      </el-button>

      <el-button size="medium" type="warning" @click="restore" :disabled="!choice" :loading="isUploading"
        >Restore</el-button
      >
    </el-row>
    <el-upload
...
const deleteBackup = async (filename: string) => {
  try {
    ...
    if (status) ElMessage.success({ showClose: true, message: `Backup deleted: ${filename}`, center: true })
    else ElMessage.error({ message: 'Could not delete backup', center: true })
    listBackup()
  } catch (error) {
    const err = <Error>error
    ElMessage.error({ message: err.message, center: true })
